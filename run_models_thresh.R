...
resThree <- parLapplyLB(cl, 1:3, function(x) {
    ...
    source('./scripts/model_fit_thresh.R')
    
    # debugonce(fitAlarmModel)
    fitAlarmModel(incData = incData, smoothC = smoothC, smoothH = smoothH,
...
source('./scripts/summarize_post_thresh.R')
...
if (!all(postSummaries$gdiag$gr < 1.1)) {
    ...
    saveRDS(resThree, 
            paste0('./output/chains_thresh_peak', 
                   peak_i, '_', smoothWindow_i, '.rds'))
}


# save results in separate files
modelInfo <- data.frame(alarmBase = 'thresh',
                        peak = peak_i,
...
saveRDS(gr, paste0('./output/gr_threshBatch', idxPrint, '.rds'))
saveRDS(paramsPost, paste0('./output/paramsPost_threshBatch', idxPrint, '.rds'))
saveRDS(alarmPost, paste0('./output/alarmPost_threshBatch', idxPrint, '.rds'))
saveRDS(R0Post, paste0('./output/R0Post_threshBatch', idxPrint, '.rds'))
saveRDS(waicPost, paste0('./output/waicPost_threshBatch', idxPrint, '.rds'))
