...
fitAlarmModel_longer <- function(incData, city, modelType, peak, timePeriod,
                                 smoothC, smoothD, hospData, deathData, 
                                 N, S0, I0, H0, D0, R0, seed) {
    
    
    source('./scripts/model_code.R')
    source('./scripts/get_model_inputs_longer.R')
    ...
    seed <- seed + 
        ...
        as.numeric(factor(city, levels = c('nyc', 'montreal'))) + 
        length(incData)
    
    set.seed(seed)
    
    # model-specific constants, data, and inits
    modelInputs <- getModelInput_longer(incData, city, modelType, peak, timePeriod,
                                        smoothC, smoothD,
                                        hospData, deathData,
                                        N, S0, I0, H0, D0, R0, 
                                        chainIdx)
