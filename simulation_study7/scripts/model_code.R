...
# Hill-Langmuir alarm function with two inputs
hillAlarm2 <- nimbleFunction(     
    run = function(x1 = double(0), x2 = double(0), 
                   nu = double(0), 
                   x0 = double(0), delta = double(0)) {
        returnType(double(0))
        
        result <- delta / (1 + (x0 / (x1 + x2)) ^ nu)
        
        return(result)
    })
assign('hillAlarm2', hillAlarm2, envir = .GlobalEnv)
...
SIHRD_sim <-  nimbleCode({
    ...
    for(t in 2:tau) {
        ...
        alarm[t] <- hillAlarm2(alpha * smoothC[t], (1 - alpha) * smoothD[t],
                                nu, x0, delta)
...
SIR_inc_undetected <-  nimbleCode({
    ...
    for(t in 1:tau) {
        ...
        alarm[t] <- hillAlarm(smoothC[t], nu, x0, delta)
