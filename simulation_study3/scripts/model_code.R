...
SIHRD_sim <-  nimbleCode({
    ...
    for(t in 2:tau) {
        ...
        alarmC[t] <- thresholdAlarm(smoothC[t], N, deltaC, HC)
        alarmH[t] <- thresholdAlarm(smoothH[t], N, deltaH, HH)
        alarmD[t] <- thresholdAlarm(smoothD[t], N, deltaD, HD)
...
SIHRD_full <-  nimbleCode({
    ...
    for (i in 1:n) {
        ...
        # compute alarms for each component
        yAlarmC[i] <- thresholdAlarm(xC[i], N, deltaC, HC)
        yAlarmH[i] <- thresholdAlarm(xH[i], N, deltaH, HH)
        yAlarmD[i] <- thresholdAlarm(xD[i], N, deltaD, HD)
