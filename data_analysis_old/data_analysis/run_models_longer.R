...
# running a specific set of models longer as they did not converge in first run
################################################################################
...
# manually select those to run longer
allModels <- allModels[c(6, 17, 18, 25, 32, 35, 40,
                         55, 60, 65, 69, 81, 85, 94),]
...
for (i in batchIdx) {
    ...
    cl <- makeCluster(3)
    clusterExport(cl, list('incData', 'city_i', 'modelType_i', 'peak_i',  'timePeriod_i',
                           'smoothC', 'smoothD', 
                           'deathData', 'hospData',
                           'N', 'S0', 'I0', 'H0', 'D0', 'R0'))
    ...
    resThree <- parLapply(cl, 1:3, function(x) {
        ...
        source('./scripts/fit_models_longer.R')
        
        # debugonce(fitAlarmModel)
        fitAlarmModel_longer(incData = incData, city = city_i,
    ...
    if (!all(postSummaries$gdiag$gr < 1.1)) {
        ...
        saveRDS(resThree, 
                paste0('./output/chains_', modelType_i, '_', city_i,
                       '_peak', peak_i, '_weeks', timePeriod_i, '_longer.rds'))
    }
...
saveRDS(gr, paste0('./output/gr_longer_Batch', idxPrint, '.rds'))
saveRDS(paramsPost, paste0('./output/paramsPost_longer_Batch', idxPrint, '.rds'))
saveRDS(alarmTimePost, paste0('./output/alarmTimePost_longer_Batch', idxPrint, '.rds'))
saveRDS(R0Post, paste0('./output/R0Post_longer_Batch', idxPrint, '.rds'))
saveRDS(waicPost, paste0('./output/waicPost_longer_Batch', idxPrint, '.rds'))
saveRDS(predFitPost, paste0('./output/predFitPost_longer_Batch', idxPrint, '.rds'))
saveRDS(predForecastPost, paste0('./output/predForecastPost_longer_Batch', idxPrint, '.rds'))
