...
SIHRD_sim <-  nimbleCode({
    ...
    # hospitalizations only from detected cases
    Hstar[1] ~ dbin(probIH, C[1])
    
    # removals from all infectious that didn't go to hospital
    RstarI[1] ~ dbin(probIR, I[1] - Hstar[1])
    
    fromH[1, 1:3] ~ dmulti(prob = fromHProb[1:3], size = H[1])
    RstarH[1] <- fromH[1, 1]
    Dstar[1] <- fromH[1, 2]
    
    # update S, I, H, R, D and C 
    S[2] <- S[1] - Istar[1]
    C[2] <- C[1] + detectIstar[1] - Hstar[1]
    ...
    for(t in 2:tau) {
        ...
        # C -> H 
        Hstar[t] ~ dbin(probIH, C[t])
        # remaining I -> R
        RstarI[t] ~ dbin(probIR, I[t] - Hstar[t])
        ...
        # update S, I, H, R, D and C 
        S[t + 1] <- S[t] - Istar[t]
        C[t + 1] <- C[t] + detectIstar[t] - Hstar[t]
...
################################################################################
# Model scripts:
#   - SIHRD model with alarm based on cases and deaths (SIHRD_full)
#   - SIHRD model with alarm based on cases only (SIHRD_inc)
#   - SIHRD model with no alarm (SIHRD_noAlarm)
# Models allow for undetected cases to be estimated (undetected)
# Second model codes for simulation are needed for SIHRD_full, SIHRD_inc because
